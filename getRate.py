...
code_path = 'src/main.cpp src/Operator.cpp src/binaryManager.cpp'
test_case_path = 'resource/testcases/'
...
with open('success_rate.md', 'w') as f:
    if full_print:
        ...
        f.write('## 分支预测成功率\n')
        f.write('| 文件名 | 总周期数 | 成功率 | 预测成功分支数 | 预测分支总数 |\n')
        f.write('| :----: | :------: | :----: | :------------: | :----------: |\n')
        for i in range(len(run_list)):
            f.write('|'+run_list[i]+'|'+true_ans[i][0]+'|'+true_ans[i][1]+'|'+true_ans[i][2]+'|'+true_ans[i][3]+'|\n')
        f.write('\n`powered by walotta`')
    else:
